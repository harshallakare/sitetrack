...
export interface LedgerEntry {
  ...
  /** Positive = increases what we owe (delivery); negative-conceptually = payment reduces it. */
  amountMinor: number;
}
 ...
 *   outstanding = delivered - paid  (what we still owe)
...
export class LedgerService {
  ...
  async vendorLedger(vendorId: string) {
    ...
    const [deliveredAgg, paidAgg, deliveries, payments] = await Promise.all([
    ...
    const entries = [...deliveryEntries, ...paymentEntries].sort(
    ...
    return {
      ...
      deliveredMinor,
      paidMinor,
      outstandingMinor: deliveredMinor - paidMinor,
      entries,
    };
  }
  ...
  async payables() {
    const [vendors, deliveredGroups, paymentsByVendor] = await Promise.all([
